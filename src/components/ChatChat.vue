<template>
  <section class="input-block">
    <textarea id="textbox" v-model="chatQst" placeholder="Let's chat !"
      @keyup.enter="textChat(chatQst, source_val, target_val)" rows="1">
    </textarea>
    <button class="btn-enter" @click="textChat(chatQst, source_val, target_val)">
      ➤
    </button>
  ...
  <section class="recording-block" v-if="record">
    ...
    <div class="recording-ended" v-else>
      <button class="btn-send" @click="AudioChat()">send recording!</button>
...
import { textChat, audioChat } from "@/assets/js/chat.js";
//[data]
...
const AudioChat = () => {
  audioChat(audioBlob.value, target_val.value);
  audioDelete();
};
